...
fn main() {
    ...
    for (model_name, model_params) in models {
        for &b_field in &b_fields {
            ...
            println!(
                "\nModelo={} | Topologia=padrão | Varrendo {} valores de \u{03BE} para B = {} G...",
                model_name, num_points, b_string
            );
            let all_results = Solver::solve_parallel(engines, 8);

            let base_dir = format!("output/nlem_log/{}/B_{}/default", model_name, b_string);
