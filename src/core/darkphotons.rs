...
impl DarkPhotonsMatter {
    ...
    fn density_fm3_to_natural(density_fm3: f64) -> f64 {
        density_fm3 / (M_NUCLEON / HBAR_C).powi(3)
    }
    ...
    fn dark_source_pressure_term(&self) -> f64 {
        self.g_d * self.n_chi * self.v_x0 / Self::kinetic_mixing_norm(self.epsilon)
    }
    ...
    pub fn with_n_chi(mut self, n_chi: f64) -> Self {
        // dark matter number density in fm^-3; internally converted to natural units
        assert!(
            n_chi.is_finite() && n_chi >= 0.0,
            "dark matter number density n_chi must be finite and non-negative"
        );
        self.n_chi = Self::density_fm3_to_natural(n_chi);
        self
    }

    pub fn with_n_chi_natural(mut self, n_chi: f64) -> Self {
        // dark matter number density in the same internal natural units as nb
        assert!(
            n_chi.is_finite() && n_chi >= 0.0,
            "dark matter number density n_chi must be finite and non-negative"
        );
    ...
    pub fn funcv(&mut self, x: &[f64]) -> [f64; 4] {
        let mixing_norm = Self::kinetic_mixing_norm(self.epsilon);
        let v_x0 = (self.g_d * self.n_chi) / (self.m_x.powi(2) * mixing_norm);
        self.v_x0 = v_x0;
...
pub fn compute(
    ...
    let press = press_sum + engine.dark_source_pressure_term() - ener;
